...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // E) - Create 6 querys 
// query #1 // List 5 films has quentin Tarantino, directed with IMDB rating of greater than 8 and show the awards those mvies have won
  db.movies.find(
      {"imdb.rating":{ $gt:8 }},
    ...
    db.movies.find({},
         ...
         {"directors":"Quentin Tarantino","awards":2,_id:0}
         ).sort({"awards":-1}
         ).limit(5).pretty()


// query #2 - List 5 movies from Horror with a imDB rating of 8 or more 
      db.movies.find(
        {"genres": "Horror"}, { "tomatoes.viewer.rating": { $lte: 4 }},
        //{_id: 0, title: 1,"imdb.rating" : 1, genres : 1}
        ).pretty() 


// query #3 - How many horror and Sci Fi  film have have a IMDB rating and a rotten tomatoes rating of less the 2 
db.movies.find( 
    {"imdb.rating": { $lt: 2 }, "tomatoes.viewer.rating": { $lt: 2 }},
  {"genres":{$all:["Thriller","Sci-Fi"]}},
  ).count()

// query #5  - List all the Comdey films that starred Tom Hanks vfrom before 2016. Show the Name, year and sort by the films with the most award wins.
db.movies.find( 
  { $and:
    [ {"genres":{$all:["Comedy"]}}, 
  {"cast":{$all:["Tom Hanks"]}} ] },
  {"title":1,"awards":1,_id:0, "year":1 }).sort({"awards.wins" :-1, },
  {
...
// query #9   - List 2 Horror Films where Boris Karloff is the cast, list the yTitle, year and Genres
db.movies.find(
     { $and:
        [{"genres":{$in:["Horror"]}},
         {"cast":{$all:["Boris Karloff"]}}]} ,
         {"title":1, "genres":1, "year":1,_id:0}
         ).limit(2).pretty()
...
// used to check if Midnigth sky was added
